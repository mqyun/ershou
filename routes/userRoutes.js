...
router.get('/fenleiSelList/:fenleiid', function(req, res, next) {
	...
	usermodel.getFenLei(function(err, fenleiList) {
		...
		for (var i = 0; i < fenleiList.length; i++) {
			if (fenleiList[i].id == fenleiid) {
				smalltip = fenleiList[i].name;
			}
		}
...
router.get('/login', function(req, res, next) {
	res.render('student/index/login', {
		title: '登录'
	});
});
			...
			usermodel.isYuDing(wupinid, studentid, function(err, isYuDing) {
				...
				usermodel.getFenLei(function(err, fenleiList) {
					...
					usermodel.getYuDingUser(wupinid, function(err, yudingUserList) {
						...
						for (var i = 0; i < wupinInfo.length; i++) {
							var sqltime = wupinInfo[i].addtime;
							var parsetime = new Date(sqltime);
							wupinInfo[i].addtime = parsetime.getFullYear() + '-' + (parsetime.getMonth() + 1) + '-' + parsetime.getDate() + ' ' + parsetime.getHours() + ':' + parsetime.getMinutes() + ':' + parsetime.getSeconds();
						}
						for (var i = 0; i < liuyanList.length; i++) {
							var sqltime = liuyanList[i].addtime;
							var parsetime = new Date(sqltime);
							liuyanList[i].addtime = parsetime.getFullYear() + '-' + (parsetime.getMonth() + 1) + '-' + parsetime.getDate() + ' ' + parsetime.getHours() + ':' + parsetime.getMinutes() + ':' + parsetime.getSeconds();
						}
...
// 查看寻物详情
router.get('/xwdetail/:id', function(req, res, next) {
	var xunwuid = req.params.id;
	usermodel.getThisXunWu(xunwuid, function(err, xunwuInfo) {
		if (err) {
			return next(err);
		}
		usermodel.getxunwuLiuYan(xunwuid, function(err, liuyanList) {
			if (err) {
				return next(err);
			}
      for (var i = 0; i < liuyanList.length; i++) {
        ...
        liuyanList[i].addtime = parsetime.getFullYear() + '-' + (parsetime.getMonth() + 1) + '-' + parsetime.getDate() + ' ' + parsetime.getHours() + ':' + parsetime.getMinutes() + ':' + parsetime.getSeconds();
			...
			res.render('student/xunwu/detail', {
				title: '寻物详情',
				xunwuInfo: xunwuInfo[0],
        liuyanList: liuyanList
			});
		});
	});
...
// 发表寻物留言
router.post('/addxunwuLiuYan', function(req, res, next) {
	var content = req.body.liuyancontent;
	var xunwuid = req.body.xunwuid;
	var studentid = req.session.uid;
	usermodel.addxunwuLiuYan(content, xunwuid, studentid, function(err) {
		if (err) {
			res.json({
				'error': err
			});
